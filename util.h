...
// Shan
struct Texture{
	Texture(bool filled , unsigned long int width, long int height, unsigned char * rarray, unsigned char* garray, unsigned char * barray) :
	filled(filled), width(width),height(height),rarray(rarray),garray(garray),barray(barray){}
...
struct Material {
	Material(Color ambient, Color diffuse, Color specular, double exp, double reflection = 0,double refraction_index=-1,Texture*texture=NULL) :
		ambient(ambient), diffuse(diffuse), specular(specular),
		specular_exp(exp), reflection(reflection), refraction_index(refraction_index),texture(texture){}
	...
	// Shan : able to do perfect reflect
	double reflection;
...
struct Intersection {
	...
	// Shan : location in model view
	Point3D model_point;
	// shape : 0->UnitSquare , 1->UnitSphere, 2->UnitCube, 3->else;
	int shape;
};
